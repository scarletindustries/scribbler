...
The WebAssembly frontend for the [carder](https://github.com/scarletindustries/carder) compiler backend — it decodes, validates and lowers wasm into carder's IR. A Gleam project (targets Erlang/BEAM by default). Built and tested with the standard Gleam toolchain — **Gleam 1.16**, Erlang/OTP, and `gleeunit` for tests.
...
- Entry point: `src/scribbler.gleam` (`pub fn main`).
- Add new modules under `src/scribbler/` and import them as `import scribbler/<module>` (e.g. `src/scribbler/wasm/decode.gleam` → `import scribbler/wasm/decode`).
...
## Relationship to carder

scribbler is the WebAssembly **frontend** for the carder compiler **backend**. scribbler owns the wasm binary/text format — decode, validate, canonicalise, lower — and stops at carder's IR. Everything from the IR down (the middle-end/optimizer, Core Erlang codegen, the BEAM runtime, the run-ABI, the shared CLI vocabulary) belongs to carder.

- **carder is an ordinary Gleam package dependency.** It is resolved through `gleam.toml`/`manifest.toml` like any other dep. Call its public API; don't reach around it.
- **NEVER vendor or fork carder source into this repo.** No copied-in modules, no "temporary" local patches to backend code. If carder needs a change, change it in carder and bump the dependency.
- **scribbler must never define a module under `src/carder/`.** Gleam hard-errors on a duplicate module path across the dependency graph, so a stray `src/carder/anything.gleam` here breaks the build for everyone.
- **Every Erlang FFI file in this repo must be named `scribbler_*_ffi.erl`.** Unlike Gleam modules, duplicate `.erl` module atoms across two packages produce **no compiler error and no warning** — one silently shadows the other, and the failure shows up as inexplicable runtime behavior. The name prefix is the only defense.

For WebAssembly semantics the authority is the [WebAssembly spec](https://webassembly.github.io/spec/) — not carder's behavior, and not whatever this code currently does. Cite it in doc comments where a decision is spec-mandated.
